    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Welcome back, Alex. Your footprint is <span className="text-primary font-bold">12% lower</span> than last week.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="glass-card col-span-1">
          ...
          <CardContent>
            <div className="flex items-center justify-between mb-4">
              <div className="text-5xl font-headline font-bold text-primary">84</div>
            ...
            <Progress value={84} className="h-2 mb-4" />
        ...
        <Card className="glass-card col-span-1 bg-primary/5 border-primary/20">
          <CardHeader className="pb-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {[
              { type: "Grocery", date: "Today, 2:45 PM", amount: "$124.50", status: "Analyzed", impact: "+5.2kg" },
              { type: "Electricity", date: "Oct 12, 2023", amount: "$89.20", status: "Verified", impact: "-2.1kg" },
              { type: "Fuel", date: "Oct 08, 2023", amount: "$45.00", status: "Flagged", impact: "+12.4kg" },
            ].map((item, i) => (
              <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-secondary/20 border border-border/50">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-background flex items-center justify-center border border-border">
                    {item.type === 'Grocery' ? <ShoppingBag className="w-5 h-5" /> : item.type === 'Electricity' ? <Zap className="w-5 h-5" /> : <Car className="w-5 h-5" />}
                  </div>
                  <div>
                    <div className="text-sm font-bold">{item.type} Receipt</div>
                    <div className="text-[10px] text-muted-foreground">{item.date}</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className={`text-xs font-bold ${item.impact.startsWith('+') ? 'text-destructive' : 'text-primary'}`}>{item.impact}</div>
